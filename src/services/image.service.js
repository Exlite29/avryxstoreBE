...
const processImage = async (imagePath, options = {}) => {
  ...
  const processedPath = path.join(os.tmpdir(), `processed_${uuidv4()}.${format}`);
...
const processBase64Image = async (base64Data, options = {}) => {
  const tempInput = path.join(os.tmpdir(), `input_${uuidv4()}.jpg`);
  const tempOutput = path.join(os.tmpdir(), `output_${uuidv4()}.webp`);
...
const generateThumbnail = async (imagePath, size = 200) => {
  const thumbnailPath = path.join(os.tmpdir(), `thumb_${uuidv4()}.webp`);
...
const autoRotateImage = async (imagePath) => {
  try {
    const rotatedPath = path.join(os.tmpdir(), `rotated_${uuidv4()}.jpg`);
