...
const recognizeFromImage = async (imagePath, options = {}) => {
  ...
  try {
    ...
    const products = await database.all(
      "SELECT id, name, category, unit_price, stock_quantity, image_urls, barcode FROM products",
    );
    ...
    for (const product of products) {
      ...
      if (matchScore > 0) {
        ...
        try {
          ...
          imageUrls = product.image_urls && product.image_urls.startsWith("http") ? [product.image_urls] : [];
        }
